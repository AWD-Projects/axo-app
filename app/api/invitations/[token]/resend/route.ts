...
export async function POST(
  ...
  const { error } = await admin
    .from("invitaciones")
    .update({ otp_code: otpHash, otp_expires_at: otpExpiresAt, otp_intentos: 0 })
    .eq("id", inv.id)
